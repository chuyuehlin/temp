...
def index():
    ...
    stockname6, id6, priceReference6, priceHighLimit6, priceLowLimit6, industryZhTw6 = homepagef(
        {"query": {"term": {"symbolId": {"value": 1218}}}})from flask import Flask, render_template, flash, redirect, url_for, session, request, logging
from wtforms import Form, StringField, TextAreaField, PasswordField, validators
from functools import wraps
import requests
import json
...
def playlist():
    ...
    cur = mysql.connection.cursor()
    result = cur.execute(
        "SELECT * FROM playlists,song_records WHERE user = %s AND song_records.songID = playlists.recordsongID", [session['username']])
...
def addtoPlaylist(song):
    ...
    cur = mysql.connection.cursor()
    result = cur.execute(
        "SELECT * FROM playlists WHERE recordsongID = %s", [song])
    if result > 0:
        ...
        cur1.execute("INSERT INTO playlists(recordsongID, user) VALUES(%s, %s)", (song, [
                     session['username']]))
        mysql.connection.commit()
        cur1.close()
        cur.close()
